...
protocol StubViewPresentable {
    ...
    func showStubView(data: BaseStubViewBuilder.InputData)
    func showStubView(stub: VisualErrorStub, buttonAction: @escaping () -> Void)
    func hideStubView()
}
...
extension StubViewPresentable {
    ...
    func showStubView(data: BaseStubViewBuilder.InputData) {
        stubViewManager.addStubView(superview: stubViewPinTo, data: data)
    }

    func showStubView(stub: VisualErrorStub, buttonAction: @escaping () -> Void) {
        let data = stub.convertToFullStubMode(buttonAction: buttonAction)
        stubViewManager.addStubView(superview: stubViewPinTo, data: data)
    }
