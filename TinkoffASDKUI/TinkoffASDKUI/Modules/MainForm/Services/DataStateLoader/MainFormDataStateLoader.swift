...
extension MainFormDataStateLoader {
    private func resolveState(
        ...
        let sbpMethodEnabled = featureToggleService.featureEnabled(.sbpMethodVisibility)
        let tpayMethodEnabled = featureToggleService.featureEnabled(.tpayMethodVisibility)

        let availableMethods = (CollectionOfOne(.card) + terminalInfo.mainFormMethods(sbpEnabled: sbpMethodEnabled, tpayEnabled: tpayMethodEnabled))
...
private extension TerminalInfo {
    func mainFormMethods(sbpEnabled: Bool, tpayEnabled: Bool) -> [MainFormPaymentMethod] {
        payMethods.compactMap { terminalMethod in
            switch terminalMethod {
            case .sbp:
                guard sbpEnabled else { return nil }
                return .sbp

            case let .tinkoffPay(method):
                guard tpayEnabled else { return nil }
