...
extension TDSFlowError: LocalizedError {
    public var errorDescription: String? {
        ...
        case let .protocolError(protocolError):
            let errorMessage = protocolError.getErrorMessage()
            let description = [
                errorMessage.getErrorCode(),
                errorMessage.getTransactionID(),
                errorMessage.getErrorDescription(),
                errorMessage.getErrorDetails(),
            ].joined(separator: " ")
            return description
        case let .runtimeError(runtimeErr):
            let description = [
                runtimeErr.getErrorCode(),
                runtimeErr.getErrorMessage(),
            ].joined(separator: " ")
            return description
        case .invalidPaymentSystem:
