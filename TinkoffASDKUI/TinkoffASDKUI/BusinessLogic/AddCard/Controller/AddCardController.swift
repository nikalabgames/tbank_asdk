...
extension AddCardController {
    ...
    private func complete3DSMethodIfNeededAndAttachCard(
        ...
        let version = check3DSPayload.receiveVersion()
        switch3dsVersion(version)
    }
    ...
    private func attachCard(
        ...
        let attachData = AttachCardData(
            ...
            requestKey: requestKey,
            data: data,
            isUsing3DS: !(messageVersion?.isEmpty ?? true)
        )
    ...
    private func confirm3DSAppBased(
        ...
        DispatchQueue.performOnMain { [weak self] in
            ...
            self?.tdsController.completionHandler = { [weak self] result in
                switch result {
                case let .success(paymentState):
                    self?.validate(
                        webViewResult: .succeded(paymentState),
                        attachPayload: attachPayload,
                        completion: completion
                    )
                case let .failure(error):
    ...
    /// Также используется для `3DS App Based` Flow
