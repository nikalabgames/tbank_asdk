...
final class TinkoffPayAppChecker: ITinkoffPayAppChecker {
    ...
    func isTinkoffPayAppInstalled() -> Bool {
        if UITestsUtils.hasConfigKey(UiTestsConfigKeys.tinkoffAppIsInstalled) {
            return true
        }
        return tinkoffPaySchemes.contains { appChecker.checkApplication(withScheme: $0) == .installed }
    }
}
